...
class HttpClient:
    ...
    async def new_tab(self, url: Optional[str]) -> TargetInfo:
        if url:
            path = f"/json/new?{aiohttp.helpers.quote(url, safe='')}"
        else:
            path = "/json/new"
        async with self.session.get(self.base + path) as resp:
            resp.raise_for_status()
    ...
    while True:
        ...
        try:
            evt = await asyncio.wait_for(
                q.get(), timeout=min(0.25, max(0.0, deadline - now))
            )
...
# ----------------------------- Command impls ----------------------------
async def cmd_list_tabs(http: HttpClient, pretty: bool) -> None:
    tabs = [t.__dict__ for t in await http.list_tabs()]
    print(json.dumps(tabs, indent=2 if pretty else None))


async def cmd_browser_info(http: HttpClient, pretty: bool) -> None:
    info = await http.browser_version()
    print(json.dumps(info, indent=2 if pretty else None))


async def cmd_new_tab(
    http: HttpClient, url: Optional[str], json_flag: bool, pretty: bool
) -> None:
...
async def cmd_close_tab(http: HttpClient, target_id: str, pretty: bool) -> None:
    res = await http.close_tab(target_id)
    print(json.dumps(res, indent=2 if pretty else None))


async def cmd_activate_tab(http: HttpClient, target_id: str, pretty: bool) -> None:
    res = await http.activate_tab(target_id)
    print(json.dumps(res, indent=2 if pretty else None))


async def cmd_navigate(
    http: HttpClient, target: str, url: str, wait: str, timeout_s: float
) -> None:
...
async def cmd_get_dom(http: HttpClient, target: str) -> None:
    ws = await http.resolve_ws_url(target)
    async with aiohttp.ClientSession() as sess:
        cdp = await CdpClient.connect(ws, sess)
        r = await cdp.send(
...
async def cmd_get_html(http: HttpClient, target: str) -> None:
    ws = await http.resolve_ws_url(target)
    async with aiohttp.ClientSession() as sess:
        cdp = await CdpClient.connect(ws, sess)
        r = await cdp.send(
...
async def cmd_get_dom_snapshot(http: HttpClient, target: str, pretty: bool) -> None:
    ws = await http.resolve_ws_url(target)
    async with aiohttp.ClientSession() as sess:
        cdp = await CdpClient.connect(ws, sess)
...
async def cmd_console_log(http: HttpClient, target: str, duration: float) -> None:
    ws = await http.resolve_ws_url(target)
    async with aiohttp.ClientSession() as sess:
        cdp = await CdpClient.connect(ws, sess)
        await cdp.send("Runtime.enable", {})
...
async def cmd_network_log(http: HttpClient, target: str, duration: float) -> None:
    ws = await http.resolve_ws_url(target)
    async with aiohttp.ClientSession() as sess:
        cdp = await CdpClient.connect(ws, sess)
        await cdp.send("Network.enable", {})
...
async def cmd_list_cookies(http: HttpClient, target: str, pretty: bool) -> None:
    ws = await http.resolve_ws_url(target)
    async with aiohttp.ClientSession() as sess:
        cdp = await CdpClient.connect(ws, sess)
        await cdp.send("Network.enable", {})
...
class BooleanOptionalAction(argparse.Action):
    ...
    def __call__(self, parser, namespace, values, option_string=None):
        ...
        setattr(namespace, self.dest, not flag.startswith("--no-"))


async def main_async(argv: Optional[List[str]] = None) -> None:
    p = argparse.ArgumentParser(
        prog="cdpctl.py", description="Python CLI for Chrome DevTools Protocol"
    )
    p.add_argument("--host", default="127.0.0.1")
    p.add_argument("--port", type=int, default=9222)
    p.add_argument(
        "--timeout", type=float, default=10.0, help="default command timeout (secs)"
    )
    p.add_argument(
        "--pretty",
    ...
    async with aiohttp.ClientSession(timeout=timeout) as session:
        ...
        if args.cmd == "list-tabs":
            ...
            await cmd_new_tab(
                http, getattr(args, "url", None), getattr(args, "json", False), args.pretty
            )
            ...
            await cmd_eval(
                http, args.id, args.expr, args.by_value, args.await_promise, args.pretty
            )
        elif args.cmd == "screenshot":
            await cmd_screenshot(
                http, args.id, args.out, args.full, args.format, args.quality
            )
        elif args.cmd == "print-pdf":
            await cmd_print_pdf(
                http, args.id, args.out, args.landscape, args.scale, args.backgrounds
            )
...
def main(argv: Optional[List[str]] = None) -> None:
    """Run the command-line interface synchronously."""
    try:
